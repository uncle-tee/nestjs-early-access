...
export class EarlyAccessController {
    ...
    public async index(@Res() res: Response) {
        let assetsDir = this.configService.viewTemplate().assetsDir || `${__dirname}/../../../../../src/early-access/assets`;
        let viewsDir = `${this.configService.viewTemplate().viewDir}/${this.configService.viewTemplate().index}` || `${__dirname}/../../../../../src/early-access/views`;
        let data = {
            showShareWithTwitter: !!this.configService.getTwitterHandle(),
            assetsDir,
        };

        let indexPath = `${viewsDir}/${this.configService.viewTemplate().index}` || `${viewsDir}/index.ejs`;
